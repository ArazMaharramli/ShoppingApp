...
namespace ShoppingApp.Repository.Implementation.Persistences.ProductPersistences
{
    public class ColorRepository : Repository<Color>, IColorRepository
    {
        ...
        public async Task<IPagedList<Color>> GetPagedAsync(Expression<Func<Color, bool>> predicate, string searchString, string sortColumn, string sortDirection, int pageColor = 10, int pageNumber = 1)
        {
            ...
            if (!(string.IsNullOrEmpty(sortColumn) && string.IsNullOrEmpty(sortDirection)))
            {
                if (sortDirection == "asc")
                {
                    switch (sortColumn)
                    {
                        case nameof(Color.UniqueTitle):
                            list = list.OrderBy(p => p.UniqueTitle);
                            break;
                        case nameof(Color.AddedDate):
            ...
            var data = await list.Skip((pageNumber - 1) * pageColor).Take(pageColor).ToListAsync();

            var pageCount = (total / pageColor) + ((total % pageColor) > 0 ? 1 : 0);
